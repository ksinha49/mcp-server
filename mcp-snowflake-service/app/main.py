"""
MCP Snowflake Server - Main Application

Implements MCP 2025-06-18 compliant server for Snowflake data warehouse access.
...
from app.provider import SnowflakeProvider
from app.tools import SNOWFLAKE_TOOLS
...
def create_mcp_server(user_token: Optional[str] = None) -> Server:
    """
    Create MCP server instance with Snowflake tools.
...
async def run_stdio():
    ...
    log.info("Starting Snowflake MCP Server in STDIO mode")
...
async def run_http(host: str, port: int):
    ...
    @app.get("/health")
    async def health():
        return {"status": "ok", "service": "snowflake-mcp-server"}
    ...
    async def protected_resource_metadata():
        ...
        return {
            ...
            "authorization_servers": [
                settings.snowflake_oauth_token_endpoint or f"https://{settings.snowflake_account}.snowflakecomputing.com/oauth/token"
            ],
    ...
    log.info("Starting Snowflake MCP Server at http://%s:%d", host, port)
