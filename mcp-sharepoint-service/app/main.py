"""
MCP SharePoint Server - Main Application

Implements MCP 2025-06-18 compliant server for Microsoft SharePoint access.
...
from app.provider import SharePointProvider
from app.tools import SHAREPOINT_TOOLS
...
def create_mcp_server(user_token: Optional[str] = None) -> Server:
    """
    Create MCP server instance with SharePoint tools.
    ...
    # Create server
    server = Server("sharepoint-mcp-server")
...
async def run_stdio():
    ...
    log.info("Starting SharePoint MCP Server in STDIO mode")
...
async def run_http(host: str, port: int):
    ...
    app = FastAPI(title="SharePoint MCP Server", version="1.0.0")
    settings = get_settings()

    @app.get("/health")
    async def health():
        return {"status": "ok", "service": "sharepoint-mcp-server"}
    ...
    log.info("Starting SharePoint MCP Server at http://%s:%d", host, port)
