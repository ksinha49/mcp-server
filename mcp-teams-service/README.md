...
MCP (Model Context Protocol) server for Microsoft Teams integration. Provides AI clients with access to Teams chats, channels, and meetings via the Microsoft Graph API.

## Overview

This service implements the MCP 2025-06-18 specification to expose Microsoft Teams functionality to AI clients like Claude Desktop. It supports both STDIO transport (for local use) and HTTP transport (for production deployments).

## Features

- **Team Management** - List and access teams
- **Channel Operations** - List channels, read/send messages
- **Chat Functionality** - Access and send chat messages
- **Meeting Support** - Retrieve meeting details
...
| Tool | Description |
|------|-------------|
| `list_teams` | List teams user has access to |
...
| `PORT` | No | Server port (default: 8003) |
