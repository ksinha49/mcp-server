"""
MCP Teams Server - Main Application

Implements MCP 2025-06-18 compliant server for Microsoft Teams access.
...
from app.provider import TeamsProvider
...
def create_mcp_server(user_token: Optional[str] = None) -> Server:
    """
    Create MCP server instance with Teams tools.
...
async def run_stdio():
    ...
    log.info("Starting Teams MCP Server in STDIO mode")
...
async def run_http(host: str, port: int):
    ...
    app = FastAPI(title="Teams MCP Server", version="1.0.0")
    settings = get_settings()

    @app.get("/health")
    async def health():
        return {"status": "ok", "service": "teams-mcp-server"}
    ...
    log.info("Starting Teams MCP Server at http://%s:%d", host, port)
...
def main():
    """Main entry point."""
    parser = argparse.ArgumentParser(description="Teams MCP Server")
    parser.add_argument(
        "--transport",
        choices=["stdio", "http"],
        default="stdio",
