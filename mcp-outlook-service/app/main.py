"""
MCP Outlook Server - Main Application

Implements MCP 2025-06-18 compliant server for Microsoft Outlook access.
...
from app.provider import OutlookProvider
from app.tools import OUTLOOK_TOOLS
...
def create_mcp_server(user_token: Optional[str] = None) -> Server:
    """
    Create MCP server instance with Outlook tools.
...
async def run_stdio():
    ...
    log.info("Starting Outlook MCP Server in STDIO mode")
...
async def run_http(host: str, port: int):
    ...
    @app.get("/health")
    async def health():
        return {"status": "ok", "service": "outlook-mcp-server"}
    ...
    log.info("Starting Outlook MCP Server at http://%s:%d", host, port)
